	...
	public static final Bitmap changeToSketch(Bitmap src, int type, int threshold) {
		...
		for (int y = 0; y < height - 2; ++y) {
			for (int x = 0; x < width - 2; ++x) {
				...
				// init color sum
				sumR = sumG = sumB = 0;
				sumR = (type * Color.red(pixels[1][1])) - Color.red(pixels[0][0]) - Color.red(pixels[0][2]) - Color.red(pixels[2][0])
				...
				sumB = (type * Color.blue(pixels[1][1])) - Color.blue(pixels[0][0]) - Color.blue(pixels[0][2]) - Color.blue(pixels[2][0])
						- Color.blue(pixels[2][2]);
				// get final Red
